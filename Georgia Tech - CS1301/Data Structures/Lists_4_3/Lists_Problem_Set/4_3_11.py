...
def two_dimensional_booleans(bool_list, use_and):
    if use_and:
        return andIsTrue(bool_list)
    else:
        return andIsFalse(bool_list)

def andIsTrue(bool_List):
    ret_list = list()
    for bools in bool_List:
        ret_list.append(all(bools))
    return ret_list

def andIsFalse(bool_List):
    ret_list = list()
    for bools in bool_List:
        ret_list.append(any(bools))
    return ret_list
