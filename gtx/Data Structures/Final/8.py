...
def check_winner(listOfLists):
    for i in range(1, len(listOfLists[0])):
        if listOfLists[0][i] != 0:
            return "Keep playing!"
    for i in range(0, len(listOfLists[1]) - 1):
        if listOfLists[1][i] != 0:
            return "Keep playing!"
    if listOfLists[0][0] > listOfLists[1][len(listOfLists[1]) - 1]:
        return "Player 1 wins!"
    elif listOfLists[0][0] < listOfLists[1][len(listOfLists[1]) - 1]:
