...
def stars(movies, tvshows):
    retDict = {}
    generator(retDict, movies)
    generator(retDict, tvshows)
    for value in retDict.values():
        value.sort()
    return retDict


def generator(retDict, production):
    for title, cast in production.items():
        for actor in cast:
            if actor not in retDict:
                retDict[actor] = []
            retDict[actor].append(title)
