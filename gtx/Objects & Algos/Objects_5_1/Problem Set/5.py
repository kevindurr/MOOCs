...
def find_net_force(listOfForce):
    total_horizontal, total_vertical = 0, 0
    for force in listOfForce:
        total_horizontal += force.get_horizontal()
        total_vertical += force.get_vertical()
    netForce = Force(round((total_horizontal ** 2 + total_vertical ** 2) ** .5, 1), round(degrees(atan2(total_vertical, total_horizontal)), 1))
    return netForce
