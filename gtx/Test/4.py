...
def check_horse_winner(tupsOfString):
    players = []
    for i in range(len(tupsOfString)):
        if len(tupsOfString[i]) != 5:
            players.append(str(i))
    if len(players) == len(tupsOfString):
        return "Everyone: keep playing!"
    elif len(players) > 1:
        return "Players " + ", ".join(players) + ": keep playing!"
    else:
        return "Player " + "".join(players) + " wins!"
