...
def check_availability(lst, proposed_time):
    date,time = str(proposed_time).split()
    for meeting in lst:
        meetdate, meettime = str(meeting.start_time).split()
        x, meetend = str(meeting.end_time).split()
        if meetdate == date:
            if time >= meettime and time <= meetend:
                return False
    return True
